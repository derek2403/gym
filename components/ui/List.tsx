...
export function List({ header, footer, children, className }: ListProps) {
  return (
    <section className={cn("mb-6", className)}>
      {header && <h2 className="text-overline mb-2.5 px-5">{header}</h2>}
      <div className="glass overflow-hidden rounded-[var(--radius-surface)]">{children}</div>
      {footer && <p className="text-caption mt-2.5 px-5 leading-relaxed">{footer}</p>}
    </section>
  );
}
  ...
  return (
    <Tag
      ...
      className={cn(
        ...
        // instead of clipping.
        "py-3.5",
        onClick && "transition-colors duration-[var(--response-fast)] active:bg-[rgba(120,120,128,0.14)] focus-visible:bg-[rgba(120,120,128,0.1)]"
      )}
    >
      {icon && (
        <span className={cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-[0.75rem]", iconBg)}>
          {icon}
        </span>
      )}
      ...
      <span
        className={cn(
          ...
          !last && "border-b-[0.5px] border-b-[rgba(60,60,67,0.08)] pb-4 -mb-1"
        )}
      >
